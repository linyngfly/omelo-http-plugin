...
Http.prototype.start = function (cb) {
  ...
  if (this.useCluster && cluster.isMaster) {
    ...
    let numReqs = {};
    setInterval(() => {
      console.log(`numReqs = ${JSON.stringify(numReqs)}`);
    }, 1000);
    ...
    cluster.on('exit', (worker, code, signal) => {
      console.log(`http Work process ${worker.process.pid} exit`);
    });

    console.log(`http cluster master ${process.pid} running`);
  } else {
    if (this.useSSL) {
    ...
    console.log(`http Work process ${process.pid} running`);
  }
}
